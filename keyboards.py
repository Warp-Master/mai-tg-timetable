        ...
        inline_keyboard=[
            [
                ...
                InlineKeyboardButton(
                    text="Да",
                    callback_data=TimetableRequest(group=group, body="week").pack(),
                ),
            ]
        ]
    )
...
def get_timetable_markup(group: str, request: str) -> InlineKeyboardMarkup:
    ...
    current_day_request = TimetableRequest(
        group=group, body="day", showing=request
    ).pack()
    current_week_request = TimetableRequest(
        group=group, body="week", showing=request
    ).pack()

    if len(request) == 4:
        prev_page = (datetime.strptime(f"{request}1", "%y%W%u") - one_day).strftime(
            "%y%W"
        )
        next_page = (datetime.strptime(f"{request}7", "%y%W%u") + one_day).strftime(
            "%y%W"
        )
        home_callback = current_week_request
        switch_type = InlineKeyboardButton(
            text="По дням", callback_data=current_day_request
        )
    elif len(request) == 5:
        ...
        switch_type = InlineKeyboardButton(
            text="По неделям", callback_data=current_week_request
        )
    else:
