...
async def command_start_handler(message: Message) -> None:
    ...
    await message.answer(
        template_env.get_template("start.html").render(username=me.username)
    )
...
async def timetable_handler(
    obj: Message | CallbackQuery | ChosenInlineResult, callback_data
):
    group, request, showing = (
        callback_data.group,
        request_processor(callback_data.body),
    ...
    if isinstance(obj, Message):
        ...
        action = partial(
            obj.bot.edit_message_text, inline_message_id=obj.inline_message_id
        )
    else:
...
async def chosen_handler(chosen_result: ChosenInlineResult):
    await timetable_handler(
        chosen_result, TimetableRequest(group=chosen_result.result_id, body="week")
    )
